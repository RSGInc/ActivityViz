        ...
        , "rgb(12, 52, 61)", "rgb(28, 69, 135)", "rgb(7, 55, 99)", "rgb(32, 18, 77)", "rgb(76, 17, 48)"]
			...
			function handleDocumentReady(callback) {
				...
				$(document).ready(function () {
						...
						var colorRamps = d3.selectAll(".ramp").on('click', function () {
								console.log('click on ramp');
								var rects = d3.select(this).selectAll("rect").each(function (d, i) {
									colors[i] = d3.rgb(d3.select(this).attr("fill"));
								});
								colorRamps.classed("selected", function () {}
									color1 = colors[0]; $("#color1").spectrum({
										color: color1.toString()
									}); color2 = colors[1]; $("#color2").spectrum({
										color: color2.toString()
									}); color3 = colors[2]; $("#color3").spectrum({
										color: color3.toString()
									}); color4 = colors[3]; $("#color4").spectrum({
										color: color4.toString()
									}); redrawMap(); updateColors($("#slider").slider("values"));
								});
							...
							var stamenTileLayer = new L.StamenTileLayer("toner-lite"); map.addLayer(stamenTileLayer); tileOptions = {
								maxZoom: 20
								, tolerance: 5
								, extent: 4096
								, buffer: 64
							...
							}); updateColors(handlers, $("#slider").slider("option", "max")); $("#attribute").change(function () {
								redrawMap();
