...
var sunburst = (function () {
			...
			d3.text(url, function (error, data) {
				...
				function updateBreadcrumbs(nodeArray, percentageString) {
					...
					d3.select("#sunburst-trail").select("#sunburst-endlabel").attr("x", (nodeArray.length + 0.5) * (b.w + b.s)).attr("y", b.h / 2).attr("dy", "0.35em").attr("text-anchor", "middle").text(percentageString);
				...
				function buildHierarchy(csv) {
					...
					for (var i = 0; i < csv.length; i++) {
						var row = csv[i];
						var parts = row.slice(0, row.length - 1);
						var size = +row[row.length - 1];
