//Based on radar-chart.js from https://github.com/alangrafu/radar-chart-d3 by Alaro Graves
//changed to accomadate both a percentValue and an original unscaled value. Each axis is scaled separately
var RadarChart = {
		defaultConfig: {
		...
		, chart: function () {
			...
			function setTooltip(tooltip, msg) {
				if (msg === false || msg == undefined) {
					...
					tooltip.select("rect").classed('visible', 1).attr("x", 0).attr("x", bbox.x - padding).attr("y", bbox.y - padding).attr("width", bbox.width + (padding * 2)).attr("height", bbox.height + (padding * 2)).attr("rx", "5").attr("ry", "5").style("fill", cfg.backgroundTooltipColor).style("opacity", cfg.backgroundTooltipOpacity);
				...
				selection.each(function (data) {
						...
						data.forEach(function (d) {
							d.axes.forEach(function (axis, i) {
								axis.x = (cfg.w / 2 - radius2) + getHorizontalPosition(i, radius2, axis.percentValue * cfg.factor);
								axis.y = (cfg.h / 2 - radius2) + getVerticalPosition(i, radius2, axis.percentValue * cfg.factor);
							});
						});
						...
						polygon.enter().append(polygonType).classed({
							area: 1
							, 'd3-enter': 1
						...
						if (cfg.circles && cfg.radius) {
							...
							var circle = circleGroups.selectAll('.circle').data(function (datum, i) {
									var axesMap = datum.axes.map(function (d) {
											//return [d, i];
											var axisObject = [
											d, i]
										return axisObject;
						...
						circle.enter().append('circle').classed({
							...
							d3.event.stopPropagation();
							setTooltip(tooltip, dd[0].axis + ': ' + Math.floor(dd[0].percentValue * 100) + '%' + ' (' + cfg.tooltipFormatValue(dd[0].value) + ')');
							//container.classed('focus', 1);
