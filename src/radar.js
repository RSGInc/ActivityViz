  ...
  radar: function radar(id, indx) {
    ...
    function createRadar() {
      ...
      if (showChartOnPage) {
        $.getJSON(dataLocation + "region.json", function(data) {
          ...
          if (data["scenarios"][scenario].visualizations != undefined) {
            if (
              data["scenarios"][scenario].visualizations["RadarCharts"][indx]
                .file
            ) {
              fileName =
                data["scenarios"][scenario].visualizations["RadarCharts"][indx]
          ...
          if (configSettings != undefined) {
            $.each(configSettings, function(opt, value) {
              if (opt == "NumberColsRadar") numberOfCols = value;
              if (opt == "IndependentScale") independentscale = value;
              if (opt == "ConvertAxesToPercent") convertAxesToPercent = value;
            });
          }
        }).complete(function() {
          ...
          if (!chartData) {
            d3.text(
              ...
              function(error, data) {
                ...
                if (error) {
                  $("#radar").html(
                    "<div class='container'><h3><span class='alert alert-danger'>Error: An error occurred while loading the radar data.</span></h3></div>"
                  );
                  throw error;
                }
                ...
                if (
                  !$.fn.DataTable.isDataTable("#" + id + "-datatable-table")
                ) {
                  var columnsDT = [];
                ...
                for (var i = 0; i < independentscale.length; i++) {
                  ...
                  independentAxesCharts.push({
                    name: scale,
                    axes: independentAxesInfo
                  });
                }
                ...
                Object.keys(axesInfo).forEach(function(key) {
                  ...
                  axisInfo.percentageScale = d3.scale
                    .linear()
                    .domain([axisInfo.min, axisInfo.max])
                    .range([0, 1]);
                });
                ...
                for (var c = 0; c < rolledUpMaps.length; c++) {
                  ...
                  Object.keys(currentrollupMap).forEach(function(chartName) {
                    ...
                    var axisOpacityScale = d3.scale
                      .linear()
                      .domain([0, 1])
                      .range(opacityScaleRange);
                    ...
                    if ($.inArray(chartName, independentscale) > -1) {
                      var indyChart = independentAxesCharts.filter(function(
                        ch
                      ) {
                      ...
                      Object.keys(indyChart[0].axes).forEach(function(key) {
                        ...
                        var radarAxisDataObject =
                          rolledUpChartNameMap[axisInfo.name];
                        ...
                        if (radarAxisDataObject == undefined) {
                          ...
                          console.log(
                            'Chart name "' +
                              chartName +
                              '" missing data for radarAxis: ' +
                              key
                          );
                        } else {
                          radarAxisDataObject.originalValue =
                            radarAxisDataObject.value;
                          if (axisInfo.max === axisInfo.min) {
                            radarAxisDataObject.value = axisInfo.percentageScale(
                              radarAxisDataObject.originalValue
                            );
                          } else {
                            radarAxisDataObject.value = axisInfo.percentageScale(
                        ...
                        radarAxisDataObject.scaledOpacity = axisOpacityScale(
                          radarAxisDataObject.value
                        );
                        chartDatumObject.sumPercentages +=
                          radarAxisDataObject.value;
                    ...
                    minSumPercentages = Math.min(
                      minSumPercentages,
                      chartDatumObject.sumPercentages
                    );
                    maxSumPercentages = Math.max(
                      maxSumPercentages,
                      chartDatumObject.sumPercentages
                    );
                ...
                var summedIdentityScale = d3.scale
                  .linear()
                  .domain([minSumPercentages, maxSumPercentages])
                  .range([0, 1]);
                var summedOpacityScale = d3.scale
                  .linear()
                  .domain([minSumPercentages, maxSumPercentages])
                  .range(opacityScaleRange);
                chartData.forEach(function(chartDatum) {
                  chartDatum.percentageBestChart = summedIdentityScale(
                    chartDatum.sumPercentages
                  );
                  chartDatum.scaledOpacity = summedOpacityScale(
                    chartDatum.sumPercentages
                  );
                });
      ...
      function createCharts() {
        ...
        var columns = radarChartContainer.selectAll(
          "#" + id + "-div .radar-column"
        );
        columns[0].forEach(function(d, columnIndex) {
          var everyThirdDataItem = chartData.filter(function(
            chartDatum,
            chartDatumIndex
          ) {
            return (
              chartDatumIndex % numColumns == columnIndex &&
              chartDatumIndex == chartDatum.chartId - 1
            ); //&& ;
          });
          var columnPortlets = d3
            .select(d)
            .selectAll(".radar-portlet")
            .data(everyThirdDataItem)
            .enter()
            .append("div")
            .attr("class", "radar-portlet");
          columnPortlets
            .append("div")
            .attr("class", "radar-portlet-header")
            .text(function(d) {
              return d.chartName;
            });
          ...
          var chartSvgs = columnPortlets
            .append("div")
            .attr("class", "radar-portlet-content")
            .attr("id", getChartId);
          var uniqueCharts = chartSvgs.forEach(function(d) {});
          chartSvgs.each(function(d) {
            RadarChart(
              "#" + getChartId(d),
              chartData.filter(function(chartDatum) {
        ...
        $(function() {
          ...
          $(".radar-portlet")
            ...
            .find(".radar-portlet-header")
            .addClass("ui-widget-header ui-corner-all")
            .prepend(
              "<span class='ui-icon ui-icon-minusthick radar-portlet-toggle'></span>"
            );
          $(".radar-portlet-toggle").on("click", function() {
            var icon = $(this);
            icon.toggleClass("ui-icon-minusthick ui-icon-plusthick");
            icon
              .closest(".radar-portlet")
              .find(".radar-portlet-content")
              .toggle();
          });
        });
        //end createCharts
