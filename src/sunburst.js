...
var svg = d3.select("#"+id+"-chart").append("svg")
    .attr("width", width)
    .attr("height", height)
    .append("g")
...
let total;
 ...
 var childData =nodeData.filter(d=> d.children == undefined);
  ...
  g.append("path")
      ...
      .style("fill", function(d) { return  color(childData.indexOf(d.data)); })
  ...
  var legend = d3.select("#"+id+"-legend")
                .append("svg")
                .attr('width', 300)
                .attr('height', 400)
                .append('g')
            ...
            legend.append("text")
                ...
                .text(function (d) {
                    return d.name
                }).style("fill","black").on('mouseover',mouseoverLeg)
                .on('mouseout',mouseout)
                .on('mousemove',mousemove);

            function mouseover(d) {
                console.log(d);
                d3.select('#'+id+'-chart').selectAll('g.arc path').transition().duration(100).style('opacity',0.2);
                d3.select('#'+id+'-chart').selectAll('path[data-legend-pos="'+childData.indexOf(d.data)+'"]').transition().duration(100).style('opacity',1);
            ...
            function mouseoverLeg(d) {
                console.log(d);
                d3.select('#'+id+'-chart').selectAll('g.arc path').transition().duration(100).style('opacity',0.2);
                d3.select('#'+id+'-chart').selectAll('path[data-legend-pos="'+childData.indexOf(d)+'"]').transition().duration(100).style('opacity',1);
               // d3.select('#'+id+'-chart').selectAll('rect[rectgrp=').transition().duration(100).style('opacity',0.2);
                showToolTipInfo(d);
            }
            function mouseout(d){
                d3.select('#'+id+'-chart').selectAll('g.arc path').transition().duration(100).style('opacity',1);
                hideToolTipSide();
            }
          ...
          var percentage = (100 * currentNode.value / total).toPrecision(3);
            ...
            d3.select("#"+id+"-current-node").text(currentNode.name);
                ...
                d3.select("#"+id+"-current-node-value").text(getReadableValueString(currentNode.value));
        ...
        function wafflePieChart() {
            ...
            var waffle = d3.select("#"+id+"-chart")
                ...
                .append("title")
                .text(function (d, i) {
                    return combineData[d.groupIndex].name + " | " + d.value.toLocaleString() + " , " + d.unitVal + "%"
                });
            ...
            function mouseover(d) {
                console.log(d);
                d3.select('#'+id+'-chart').selectAll('rect').transition().duration(100).style('opacity',0.2);
                d3.select('#'+id+'-chart').selectAll('rect[rectgrp='+combineData[d.groupIndex].name+']').transition().duration(100).style('opacity',1);
               // d3.select('#'+id+'-chart').selectAll('rect[rectgrp=').transition().duration(100).style('opacity',0.2);
                showToolTipInfo(d);
            }
            function mouseoverLeg(d) {
                console.log(d);
                d3.select('#'+id+'-chart').selectAll('rect').transition().duration(100).style('opacity',0.2);
                d3.select('#'+id+'-chart').selectAll('rect[rectgrp='+d.name+']').transition().duration(100).style('opacity',1);
               // d3.select('#'+id+'-chart').selectAll('rect[rectgrp=').transition().duration(100).style('opacity',0.2);
                showToolTipInfo(d);
            }
            function mouseout(d){
                d3.select('#'+id+'-chart').selectAll('rect').transition().duration(100).style('opacity',1);
                hideToolTipSide();
            }
            function mousemove(d){
	...
	if(showChartOnPage){
	     $.getJSON(dataLocation + "region.json", function (data) {
            ...
            if (data["scenarios"][scenario].visualizations != undefined) {
                ...
                if (data["scenarios"][scenario].visualizations["Sunburst"][indx].config) {
                    configName = data["scenarios"][scenario].visualizations["Sunburst"][indx].config;
                }
