  ...
  three3dFunction: function three3dFunction(id, indx) {
    ...
    function getTheConfigFile(callback) {
      $.getJSON(dataLocation + "region.json", function(data) {
        ...
        $.each(data, function(key, val) {
          if (key == "ZoneFile") zonefiles = val;
          if (key == "CenterMap" && CENTER_MAP.length == 0) CENTER_MAP = val;
        });
        if (
          ...
          function(error, filterdata) {
            ...
            for (var i = 0; i < zoneheaders.length; i++) {
              if (zoneheaders[i] in zonefilters) {
                $("#" + id + "-filter-checkboxes").append(
                  '<li><label > <input type="checkbox" colname="' +
                    zoneheaders[i] +
                    '" id="' +
                    zoneheaders[i] +
                    '_id" checked>' +
    ...
    function readInData(callback) {
      ...
      d3.text(url, function(error, data) {
        ...
        if (error) {
          $("#" + id + "-div").html(
            "<div class='container'><h3><span class='alert alert-danger'>Error: An error occurred while loading the 3D Map data.</span></h3></div>"
          );
          throw error;
        }
        //CHART ALREADY SET
        ...
        var rolledUpMap = d3
          ...
          .key(function(d) {
            ...
            var period;
            if (DataHasPeriods) {
              period = d[PERIOD_COLUMN] = parseInt(
                d[PERIOD_COLUMN].replace(/\D/g, "")
              );
            } else {
              period = d[PERIOD_COLUMN] =
                $.inArray(d[PERIOD_COLUMN], periodNames) + 1;
            }
            ...
            if (Object.keys(zonefilters).length > 1) {
              for (var i in zonefilters) {
                zoneData[zone].filters[i] =
                  zoneFilterData.filters[zone - 1][zoneheaders.indexOf(i)];
              }
            }
        ...
        var selectList = periodNames.forEach(function(d, i) {
        ...
        allTimeLinearScale = d3.scale
          .linear()
          .domain(minMaxRange)
          .range([0, 1]);
        allTimeSqrtScale = d3.scale
          .sqrt()
          .domain(minMaxRange)
          .range([0, 1]);
        zoneFilterData = null;
    ...
    function addZoneGeoJSONToMap() {
      zoneDataLayer = VIZI.geoJSONLayer(zoneGeoJSON, {
        ...
        filter: function(feature, layer) {
          ...
          var checkedfilters = $(
            "#" + id + "-filter-checkboxes input[type=checkbox]:checked"
          );
          var cnttrue = 0;
    ...
    function createMap(callback) {
      ...
      if (logEvents) {
        ...
        map.emit = function() {
          ...
          if (
            ["preUpdate", "postUpdate", "layerAdded"].indexOf(eventName) == -1
          ) {
      ...
      $.getJSON(dataLocation + "/" + ZONE_FILE_LOC, function(zoneTiles) {
        ...
        zoneGeoJSON.features.forEach(function(feature) {
          ...
          var latLngs = abmviz_utilities.flatByOne(feature.geometry.coordinates);
    ...
    function initializeMuchOfUI() {
      ...
      $("#" + id + "-centroids").change(function() {
        ...
        zoneGeoJSON.features.forEach(function(feature) {
          feature.geometry.coordinates = drawCentroids
            ? feature.centroidCoordinates
            : feature.perimeterCoordinates;
        }); //end zoneGeoJSON forEach
        redrawMap();
      }); //end change handler
      $("#" + id + "-controls").change(function() {
        showControls = this.checked;
        if (showControls) $(".control button").show();
        else $(".control button").hide();
      });
      ...
      function updateTimeSliderTooltip(value) {
        ...
        $("#" + id + "-slider-time .ui-slider-handle:first").html(
      ...
      var doesntexist =
        $("#" + id + "-period-id option[value=" + currentPeriod + "]").val() ===
        undefined;
      $("#" + id + "-slider-time").slider({
        ...
        create: function(e, ui) {
          var doesntexist =
            $(
              "#" + id + "-period-id option[value=" + currentPeriod + "]"
            ).val() === undefined;
      ...
      $("#" + id + "-stop-cycle-map").click(function() {
        ...
        $("#" + id + "-start-cycle-map").css(
          "display",
          DataHasPeriods == false ? "none" : "inline"
        );
      });
      ...
      function cyclePeriod() {
        ...
        if (cycleGoing) {
          ...
          setTimeout(
            cyclePeriod,
            Math.max(0, timeInterval - elapsedTimeSinceCycle)
          );
        } //end if cycleGoing
      } //end cyclePeriod
    ...
    function updateCurrentPeriodOrClassification() {
      "use strict";
      $("#" + id + "-current-period").html(
        abmviz_utilities.halfHourTimePeriodToTimeString(currentPeriod)
      );
