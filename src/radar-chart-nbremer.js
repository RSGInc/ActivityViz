...
function RadarChart(id, data, options) {
  ...
  axisGrid
    ...
    .attr("dy", "0.4em")
    .style("font-size", cfg.circleLabelSizes + "px")
    .attr("fill", "#737373")
    .text(function(d, i) {
  ...
  axis
    .append("text")
    .attr("class", "legend")
    .style("font-size", cfg.axisLabelSizes + "px")
      ...
      data.forEach(function(d, idx) {
        if (d["active"] == true) {
          ...
          text +=
            ...
            "</td><td class='value'> " +
            Format(
              cfg.convertAxesToPercent
                ? d.axes[i].value
                : d.axes[i].originalValue
            ) +
            " (" +
            cfg.tooltipFormatValue(d.axes[i].originalValue) +
            ")</td></tr>";
        }
      });
      text += "</tbody></table>";
  ...
  blobWrapper
    ...
    .attr("cx", function(d, i) {
      return (
        rScale(cfg.convertAxesToPercent ? d.value : d.originalValue) *
        Math.cos(angleSlice * i - Math.PI / 2)
      );
    })
  ...
  var blobCircleWrapper = g
    ...
    .attr("radar", function(d) {
      return (
        "#radar-" +
        d["chartId"] +
        " .areaName" +
        d[areaName].replace(/\s+/g, "")
      );
    });
  ...
  blobCircleWrapper
    ...
    .on("mouseover", function(d, i) {
      d3.selectAll(".radarArea")
        ...
        .style("fill-opacity", 0.1);
      var rdSelector = $(this)
        .closest(".radarCircleWrapper")
        .attr("radar");

      d3.select(rdSelector)
        .transition()
        .duration(200)
        .style("fill-opacity", 0.7);
    })
  ...
  function cellover(d) {
    //Dim all blobs
    d3.selectAll("#radar-" + data[d]["chartId"])
      .selectAll(".radarArea")
      .transition()
      .duration(200)
      .style("fill-opacity", 0.1);
  ...
  svg
    ...
    .call(function(d) {
      data.forEach(function(chart, j) {
        var legend = d3
          .selectAll("#radar-" + chart.chartId + " path.swatch")
          .filter(function(d, i) {
            return i === j;
          });
